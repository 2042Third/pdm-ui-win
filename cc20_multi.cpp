...
int DISPLAY_PROG =0;
uint8_t* win_addr_track[THREAD_COUNT];
...
//  cout<<"pos5"<<endl;
  ...
  set_thread_arg(np % THREAD_COUNT, (uint8_t*)linew, tracker, n, 0, line, count, this);
  threads[np % THREAD_COUNT] = thread(multi_enc_pthrd, tmpn);
  np++;
//  cout<<"pos6"<<endl;
...
//          #endif
        ...
        set_thread_arg(np % THREAD_COUNT, (uint8_t*)linew+tn, tracker, n, tn, line + tn, count + 1, this);
        threads[np % THREAD_COUNT] = thread(multi_enc_pthrd, np % THREAD_COUNT);
...
void multi_enc_pthrd(int thrd) {
  uint8_t * linew1 = (uint8_t*)win_addr_track[thrd]; // Set but not used
